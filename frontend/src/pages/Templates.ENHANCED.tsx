      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <div className="relative max-w-md">
            ...
            />
            <svg 
              className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-monday-textLight dark:text-gray-400" 
              fill="none" 
              stroke="currentColor" 
              viewBox="0 0 24 24"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredTemplates.map((template) => (
            <div
              key={template.id}
              className="bg-white dark:bg-monday-darkLight border border-monday-border dark:border-gray-700 rounded-xl p-6 hover:shadow-2xl hover:border-monday-primary transition-all group"
            >
              {/* Icon */}
              ...
              <p className="text-sm text-monday-textLight dark:text-gray-400 mb-4 min-h-[40px]">
              ...
              <div className="flex items-center gap-4 mb-4 text-xs text-monday-textLight dark:text-gray-400">
                <span className="flex items-center gap-1">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 0v10m0-10a2 2 0 012 2h2a2 2 0 012-2m0 0v10a2 2 0 01-2 2h-2a2 2 0 01-2-2" />
                  </svg>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 px-4 py-2 bg-monday-primary text-white rounded-lg font-medium hover:bg-monday-primaryHover transition-colors text-sm"
                >
                  Use Template
                </button>
                <button
                  onClick={() => handlePreviewTemplate(template)}
                  className="px-4 py-2 border border-monday-border dark:border-gray-700 text-monday-text dark:text-white rounded-lg font-medium hover:bg-monday-background dark:hover:bg-gray-800 transition-colors text-sm"
        ...
        {filteredTemplates.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-lg text-monday-textLight dark:text-gray-400 mb-2">
              {searchQuery ? 'No templates match your search' : 'No templates found in this category'}
            </p>
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="text-monday-primary hover:underline"
              >
                Clear search
              </button>
            )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-monday-darkLight rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-monday-textLight dark:text-gray-400">
                    {previewTemplate.category} • {previewTemplate.columns.length} columns • {previewTemplate.groups.length} groups
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-6">
              ...
              {/* Columns Preview */}
              <div className="mb-6">
                <h3 className="text-lg font-bold text-monday-text dark:text-white mb-3">
                  Columns ({previewTemplate.columns.length})
                </h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {previewTemplate.columns.map((column, index) => (
                    <div
                      key={index}
                      className="p-3 bg-monday-background dark:bg-gray-800 rounded-lg border border-monday-border dark:border-gray-700"
                    >
                      <div className="font-medium text-monday-text dark:text-white text-sm">
                        {column.name}
                      </div>
                      <div className="text-xs text-monday-textLight dark:text-gray-400 capitalize">
                        {column.type}
                      </div>
                    </div>
                  ))}
              ...
              <div>
                <h3 className="text-lg font-bold text-monday-text dark:text-white mb-3">
                  Groups ({previewTemplate.groups.length})
                </h3>
                <div className="space-y-2">
                  {previewTemplate.groups.map((group, index) => (
                    <div
                      key={index}
                      className="p-3 bg-monday-background dark:bg-gray-800 rounded-lg border border-monday-border dark:border-gray-700 flex items-center gap-2"
                    >
                      <svg className="w-4 h-4 text-monday-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" />
                      </svg>
                      <span className="font-medium text-monday-text dark:text-white text-sm">
                        {group.name}
                      </span>
                    </div>
                  ))}
            ...
            {/* Modal Footer */}
            <div className="p-6 border-t border-monday-border dark:border-gray-700 flex gap-3">
              <button
                onClick={() => {
                  setShowPreview(false)
                  setPreviewTemplate(null)
              ...
              <button
                ...
                className="flex-1 px-4 py-3 bg-monday-primary text-white rounded-lg font-medium hover:bg-monday-primaryHover transition-all"
              >
                Use This Template
              </button>
            </div>
